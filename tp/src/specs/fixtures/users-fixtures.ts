...
type UserFixtureOptions = Partial<
...
export function buildUserFixture(opts: UserFixtureOptions = {}) {
    ...
    user.email =
        opts.email?.toLocaleLowerCase() ?? faker.internet.email().toLowerCase()

    // that hash matches password 'changethat', hardcoded so we save CPU hasing time
    user.passwordHash =
        '$2a$12$dm2t30Y07Mt9TklkLOuy.efFIJ69WTW3f7NmwH8uioX9R6NHMQSXO'
    user.sessions = []
